...
var ReaddirpStream = class extends Readable {
  ...
  async _formatEntry(dirent, path3) {
    ...
    try {
      const fullPath = presolve(pjoin(path3, basename5));
...
var NodeFsHandler = class {
  ...
  _watchWithNodeFs(path3, listener) {
    ...
    const basename5 = sp.basename(path3);
...
var Watcher = class {
  ...
  add_watch(watch_id, path3) {
    add(this.id_to_watching_path, watch_id, path3);
    add(this.watching_path_to_id, path3, watch_id);
  }
  start_watching() {
    for (const [watching_path, ids] of Object.entries(this.watching_path_to_id)) {
      const watcher = watch(watching_path).on("change", (path3) => {
        for (const id of ids)
          add(this.id_to_changed_path, id, path3);
      });
