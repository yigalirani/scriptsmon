...
var TreeControl = class {
  ...
  <div class="tree_${type}" id="${id}" >
    ...
    if (start_animation) {
      const animate = ans.querySelectorAll("animate");
...
function convert(root, old_root) {
  ...
  function f(node) {
    ...
    const items = node.runners.map((runner) => {
      const { script, state, id: id2 } = runner;
      const start_animation = changed_ids.has(id2);
      const ans2 = { type: "item", id: id2, label: name, commands: ["play", "debug"], children: [], description: script, icon: state, start_animation };
      return ans2;
    });
