...
<svg class="Runningicon" xmlns = "http://www.w3.org/2000/svg" viewBox = "0 0 100 100" preserveAspectRatio = "xMidYMid" width = "233" height = "233" fill="none" xmlns:xlink = "http://www.w3.org/1999/xlink">
  ...
  <div class="icon">folder_obsolute
...
var build_in_filters = ["All", "Watchable", "Watching", "Ready", "Running", "Done", "Stopped", "Warning", "Error", "Error or Warning"];
function is_filter(x) {
  if (build_in_filters.includes(x))
    return true;
  return is_tag(x);
}
function get_filter() {
  const value = document.body.querySelector("#status_filter")?.value || "";
  if (!is_filter(value))
    return "All";
  return value;
}
function the_convert(report) {
  ...
  function collect_toggles(items) {
    const ans = {};
